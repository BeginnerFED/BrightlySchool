...
export default function UpdateModal({ isOpen, onClose, onSuccess, registration }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (findLatestError) {
        ...
        if (findLatestError.code !== 'PGRST116') {
          ...
          if (insertFinancialError) {
            ...
            setToast({
              visible: true,
              message: language === 'uk' ? 'Помилка при збереженні фінансових даних.' : 'Error saving financial details.',
              type: 'error'
            });
          }
        }
        ...
        if (updateFinancialError) {
          ...
          setToast({
            visible: true,
            message: language === 'uk' ? 'Помилка при оновленні фінансових даних.' : 'Error updating financial details.',
            type: 'error'
          });
      ...
      setToast({
        visible: true,
        message: language === 'uk' ? 'Запис успішно оновлено.' : 'Record has been successfully updated.',
        type: 'success'
      })
      onClose()
      onSuccess?.()
